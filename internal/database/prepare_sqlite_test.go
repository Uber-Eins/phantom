...
func TestPrepareSQLiteForMigration(t *testing.T) {
	t.Run("rejects non-panel database", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "not a phantom panel database") {
			t.Fatalf("error = %q, want it to contain %q", err.Error(), "not a phantom panel database")
		}
	})
	...
	t.Run("upgrades old panel schema", func(t *testing.T) {
		...
		for _, table := range []string{"outbound_subscriptions"} {
