...
func TestPgRestoreReadFailureError(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			localVersion: "16.4",
			want:         "This backup was created by pg_dump from PostgreSQL 17 or newer, but pg_restore version 16.4 cannot read it; provide postgresql-client 17 or newer in the runtime environment, then retry the import",
		},
		{
			name:         "dump from postgres 16 on older client",
			probeOutput:  "pg_restore: error: unsupported version (1.15) in file header",
			localVersion: "15.8",
			want:         "This backup was created by pg_dump from PostgreSQL 16 or newer, but pg_restore version 15.8 cannot read it; provide postgresql-client 16 or newer in the runtime environment, then retry the import",
		},
		{
		...
		},
		{
			...
			probeOutput:  "pg_restore: error: unsupported version (1.16) in file header",
			localVersion: "",
			want:         "This backup was created by pg_dump from PostgreSQL 17 or newer, but pg_restore version unknown cannot read it; provide postgresql-client 17 or newer in the runtime environment, then retry the import",
		},
		{
