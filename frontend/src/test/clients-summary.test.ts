...
describe('pickClientsSummary', () => {
  ...
  it('keeps the server summary when the snapshot is short of the server total', () => {
    const shortSnapshot: Row[] = Array.from({ length: 58 }, (_, i) => row({ email: `c${i}@x`, enable: true }));
    const summary = pickClientsSummary(serverSummary, shortSnapshot, new Set(), 3 * DAY, 1 * GB);
    expect(summary).toEqual(serverSummary);
  });
  ...
  it('uses the live recompute when the snapshot covers every client', () => {
    ...
    const summary = pickClientsSummary(serverSummary, fullSnapshot, new Set(), 3 * DAY, 1 * GB);
    expect(summary.total).toBe(67);
    expect(summary.active).toBe(67);
  });

  it('falls back to the server summary before the first snapshot arrives', () => {
    const summary = pickClientsSummary(serverSummary, [], new Set(), 3 * DAY, 1 * GB);
    expect(summary).toEqual(serverSummary);
  });
});
